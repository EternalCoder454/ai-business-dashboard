 ...
 * these can be rewritten or deleted in Admin.
 */
interface SeedPage {
  slug: string;
  title: string;
...
One AI per department. Each has its own area, its own conversations, and the company's context, so you do not have to explain the business every time you ask something.
...
**Ask the right department.** Pricing goes to Finance, a landing page to Marketing. Ask the wrong one and you get a redirect instead of the work.
...
**Do not paste secrets.** No passwords, API keys, card numbers, or customer personal data. It is sent to a third-party model and stored in the conversation.
...
Ask Everyone puts one question to every department simultaneously, with an optional summary read across the answers. It costs roughly one reply per department, so it is worth it for a question that genuinely crosses the whole company and wasteful otherwise.

Each answer is capped to a word count, set in Settings. For depth, use a normal conversation.

## What a department can do

Besides replying, a department can propose an action: creating a task, recording a decision or a figure, saving a deliverable. Nothing runs on its own. The proposal appears in the conversation as something to approve, and only runs when you approve it.`,
  },
  {
    slug: "memory",
    title: "Memory",
...
**Keep entries short.** Every live entry is added to every prompt.
...
Three columns: To do, Doing, Done. Drag a card between them or tick it off in place.
...
New tasks go to the top of their column. Due dates are optional, and anything overdue turns red and appears in notifications. Completed tasks stay on the board and leave the prompt.`,
  },
  {
    ...
    body: `A skill is a written playbook: a name, a trigger line saying when it applies, and a body saying how to do the work. Every enabled skill is added to that department's prompt in full, on every message.

## Writing one

**The trigger line matters most.** It is what gets matched. "Use when pricing a client website" beats "pricing guidance".
...
Groups conversations, deliverables, files, tasks, and decisions that belong to the same piece of work. Deleting a project releases its contents rather than deleting them.

A project can be shared with another approved account, which makes its conversations collaborative.

## Library

**Files** are images, PDFs, and documents attached to any conversation without re-uploading. They can be scoped to one department.
...
Direct messages with colleagues on this workspace, rather than with a department. Recorded like everything else.`,
  },
  {
    slug: "cost",
    title: "Cost",
...
**Caching does most of the work.** The context block is cached, so later messages in a session cost a fraction of the first.
